...
| LLM | Anthropic Claude — live for host **banter localization** (non-en/it narrator languages); mood/theme and generated round direction still planned |
| TTS | ElevenLabs |
| Lyrics | Musixmatch |
...
| Anthropic | Claude | **Live for banter localization** via raw `fetch` to `POST /v1/messages` (no `@anthropic-ai/sdk` dependency) in `lib/server/anthropic.ts`; default model `claude-opus-4-8` (`ANTHROPIC_BANTER_MODEL` override). English/Italian use static packs in `lib/game/host-banter.ts`; the other 27 narrator languages are Claude-generated once and cached, falling back to English if `ANTHROPIC_API_KEY` is unset. Round generation and mood/theme are ⏳ Planned |
