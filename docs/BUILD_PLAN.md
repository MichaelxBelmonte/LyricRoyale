...
> **Status — target architecture, not current build.** This doc describes the *planned* 5-day build, not what ships today. **Supabase (Postgres/RLS/leaderboards), Anthropic Claude round generation and mood, async `share_slug` challenges, and the `/api/round/generate`, `/api/host/banter`, `/api/mood`, `/api/challenge/*`, `/api/stems`, `/api/mxm/lyrics|subtitle|match` routes are PLANNED, not built.** What is live today: in-memory session store with ~1s HTTP polling (no DB), real Musixmatch/ElevenLabs/LALAL.AI proxies, server-side answer regeneration, 9 mini-games, and templated host banter — English/Italian use built-in static `{placeholder}` packs, and the other 27 narrator languages are localized once by Claude (raw `fetch`, no SDK) with an English fallback when no key is set (`lib/server/anthropic.ts`). Round generation and mood remain non-LLM/planned. See `../README.md` ("Status & known limitations") for exactly what runs today.
...
- **LLM = Claude `claude-opus-4-8`** (default model; override via `ANTHROPIC_BANTER_MODEL`). Round generation and mood/theme analysis are ⏳ **Planned** — those answers are regenerated server-side without an LLM (`/api/rounds/check`, `/api/rounds/finish-line`). **Host banter localization is partially built:** there is no `@anthropic-ai/sdk` dependency, but `lib/server/anthropic.ts` calls Claude via raw `fetch` (`POST https://api.anthropic.com/v1/messages`, structured outputs via `output_config.format` `json_schema`) to translate the static English banter pack into a chosen narrator language. English/Italian are built-in static packs (`lib/game/host-banter.ts`) and never hit Claude; the other 27 languages are generated once and cached module-global, falling back to the English pack when `ANTHROPIC_API_KEY` is absent or the call fails. Claude only ever sees/returns `{placeholder}` templates — never live player data. Planned round/mood Claude calls would request strict JSON.
...
2. **AI host — banter generation (Claude) — ⏳ partially built.** *Today* host banter is `{placeholder}` templates in `lib/game/host-banter.ts`, interpolated in code so the LLM never touches live player data. English/Italian ship as static packs; for the other 27 narrator languages, `resolveBanterPack` (`lib/server/anthropic.ts`) calls Claude (`claude-opus-4-8`, raw `fetch`, structured outputs) once per language at session creation to translate the English pack, caches it module-global, and falls back to English when no key/call fails. ⏳ **Planned (not built):** event-specific Claude-*written* banter and a selectable persona (Hype-Man / Deadpan British Judge / Diva), stored on `profiles.host_persona`, speaking at round intro, correct answer, wrong answer, score reveal, game outro. Lines are short (1–2 sentences), punchy, no slurs, broadly tasteful.
...
### Day 3 done when
Finishing a game yields a working `share_slug` link a friend can play on the same rounds; the AI host speaks persona-appropriate banter at each game event. ⏳ **Note:** the `share_slug` flow and event-specific Claude-*written* banter (with personas) are Planned; today banter is template-based (`lib/game/host-banter.ts`) — static EN/IT packs, and Claude-*localized* packs for the other 27 languages (`lib/server/anthropic.ts`) — and ElevenLabs-voiced via `/api/host/speak`.
