...
const FIELD =
  "h-14 w-full rounded-xl border border-black/15 bg-white px-4 text-xl text-[#0b0b0b] outline-none transition-colors placeholder:text-black/35 focus:border-[#ff007f] focus:shadow-[0_0_0_3px_rgba(255,0,127,0.15)]";
...
export default function PlayerRoom({ code }: { code: string }) {
    ...
    return (
      <main className="mx-auto flex min-h-[100dvh] w-full max-w-md flex-col justify-center px-4 py-10">
        <JCard spine="PLAYER · SIDE B" contentClassName="p-7 text-center">
          ...
          <h1 className="mt-4 font-condensed text-3xl uppercase tracking-tight text-[#0b0b0b]">
            Join required
          </h1>
  ...
  return (
    <main className="mx-auto flex min-h-[100dvh] w-full max-w-md flex-col px-4 py-5">
      ...
      <section className="flex flex-1 flex-col justify-center py-6">
        {error ? (
          <div className="mb-3 rounded-xl border border-[#ff007f]/30 bg-[#ff007f]/5 p-3 text-sm text-[#ff8cc0]">
            <p>{error}</p>
            <Link
              href="/"
        ...
        {session?.status === "playing" && session.currentRound ? (
          <JCard spine="LOCK IN · SIDE B" contentClassName="p-6">
            ...
            <h1 className="mt-4 font-condensed text-3xl uppercase leading-tight tracking-tight text-[#0b0b0b]">
            ...
            {answer ? (
              <div className="mt-6 rounded-xl border border-black/12 bg-white/70 p-5 text-center">
                  ...
                  className={
                    session.currentRound.status === "revealed"
                      ? answer.correct
                        ...
                        : "font-condensed text-sm uppercase tracking-[0.12em] text-[#d80069]"
                ...
                <p className="mt-1 font-condensed text-5xl tabular-nums text-[#0b0b0b]">
              ...
              <div className="mt-6 grid gap-2">
                {session.currentRound.options?.map((option, index) => (
                  <button
                    ...
                    disabled={submitting}
                    className="flex min-h-14 w-full items-center gap-3 rounded-xl border-2 border-black/15 bg-white px-4 py-3 text-left text-base font-semibold text-[#0b0b0b] transition-colors hover:border-[#ff007f] disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    <span className="font-mono text-sm text-black/40">{index + 1}</span>
                    {option}
                  </button>
                ))}
              </div>
                ...
                return (
                  <div
                    ...
                    className={[
                      ...
                      mine ? "border-[#ff007f]/40 bg-[#ff007f]/10" : "border-black/10 bg-white/60",
                    ].join(" ")}
                  >
                    <span className="font-mono text-xs text-black/40">{index + 1}</span>
                    <span className={mine ? "truncate font-semibold text-[#0b0b0b]" : "truncate text-black/70"}>
                      {item.playerName}
                    </span>
                    <span className="font-mono text-sm tabular-nums text-[#0b0b0b]">{item.points}</span>
                  </div>
                );
              })}
            </div>
...
function Waiting({ title, body }: { title: string; body: string }) {
  return (
    <JCard contentClassName="p-7 text-center">
      ...
      <h2 className="mt-4 font-condensed text-3xl uppercase tracking-tight text-[#0b0b0b]">{title}</h2>
